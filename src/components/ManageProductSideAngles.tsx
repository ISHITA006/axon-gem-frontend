...
import CreateProductSideAngleDialog from "@/components/CreateProductSideAngleDialog";

export default function ManageProductSideAngles() {
  const { token } = useAuth();
  const { toast } = useToast();

  const [angles, setAngles] = useState<ProductSideAngleRecord[]>([]);
  ...
  const fetchAngles = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: err instanceof Error ? err.message : "Failed to load product side angles",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (uid: string) => {
    ...
    setDeletingUid(uid);
    try {
      await apiDeleteProductSideAngle(token, uid);
      toast({ title: "Deleted", description: "Product side angle removed." });
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">Manage Product Side Angles</CardTitle>
            <Button size="sm" onClick={() => setIsCreateModalOpen(true)}>
              <Plus className="mr-2 h-4 w-4" />
              Add Product Side Angle
            </Button>
          </div>
        </CardHeader>
        <CardContent>
          <p className="mb-4 text-sm text-muted-foreground">
            Product side angles are reference poses used when generating side-view studio product
            shoots. Select one during Product Shoot when side view generation is enabled.
          </p>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-muted-foreground">No product side angles yet</p>
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
              {angles.map((angle) => (
      ...
      <AlertDialog open={!!confirmDeleteUid} onOpenChange={() => setConfirmDeleteUid(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Product Side Angle</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete this product side angle and its image.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
